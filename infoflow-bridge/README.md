...
```bash
MULTICA_SERVER_URL=http://localhost:8080 multica --profile 名字 login --token mul_...
multica --profile 名字 config set workspace_id <工作区 id>
```

profile 叫什么无所谓，配对靠的是这个 profile 登录进 Multica 的邮箱。所以一个人第一次用这个页面，得先在终端里
以自己的账号登录一个 profile，否则页面会告诉他「这台机器上没有以 <邮箱> 登录的 Multica 账号」。
