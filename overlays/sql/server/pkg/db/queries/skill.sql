...
SELECT * FROM skill
WHERE scope IN ('workspace', 'team') OR (scope='personal' AND owner_user_id=sqlc.arg('owner_user_id')::uuid)
ORDER BY name ASC;
...
WHERE (s.scope IN ('workspace', 'team') OR (s.scope='personal' AND s.owner_user_id=sqlc.arg('owner_user_id')::uuid))
  AND sqlc.arg('workspace_id')::uuid IS NOT NULL
...
-- name: IncrementSkillUsageCounts :exec
INSERT INTO skill_usage_stats (skill_id, usage_count, updated_at)
SELECT id, 1, now()
FROM skill
WHERE id = ANY(sqlc.arg('skill_ids')::uuid[])
...
SELECT * FROM skill
WHERE id = $1 AND (scope IN ('workspace', 'team') OR (scope='personal' AND owner_user_id=sqlc.arg('owner_user_id')::uuid))
  AND sqlc.arg('workspace_id')::uuid IS NOT NULL;
...
-- name: GetSkillByScopeAndName :one
SELECT * FROM skill
WHERE name = sqlc.arg('name')
  AND (
    (sqlc.arg('scope')::text = 'team' AND scope = 'team')
    OR (sqlc.arg('scope')::text = 'workspace' AND scope = 'workspace' AND workspace_id = sqlc.arg('workspace_id')::uuid)
    OR (sqlc.arg('scope')::text = 'personal' AND scope = 'personal' AND owner_user_id = sqlc.arg('owner_user_id')::uuid)
  );

-- name: CreateSkill :one
INSERT INTO skill (workspace_id,name,description,content,config,created_by,scope,owner_user_id)
VALUES ($1,$2,$3,$4,$5,$6,sqlc.arg('scope'),sqlc.arg('owner_user_id')::uuid)
RETURNING *;
...
DELETE FROM skill WHERE id = $1 AND (scope IN ('workspace', 'team') OR (scope='personal' AND owner_user_id=sqlc.arg('owner_user_id')::uuid))
  AND sqlc.arg('workspace_id')::uuid IS NOT NULL;

-- name: ChangeSkillScope :one
UPDATE skill SET
  scope = sqlc.arg('scope'),
  workspace_id = CASE WHEN sqlc.arg('scope') = 'team' THEN NULL ELSE sqlc.arg('workspace_id')::uuid END,
  owner_user_id = CASE WHEN sqlc.arg('scope') = 'personal' THEN sqlc.arg('owner_user_id')::uuid ELSE NULL END,
  updated_at = now()
WHERE id = sqlc.arg('id')::uuid AND created_by = sqlc.arg('creator_id')::uuid
RETURNING *;

-- name: GetSkillForAgentBinding :one
SELECT * FROM skill WHERE id=$1 AND (scope IN ('workspace', 'team') OR (scope='personal' AND owner_user_id=sqlc.arg('owner_user_id')::uuid))
  AND sqlc.arg('workspace_id')::uuid IS NOT NULL;
