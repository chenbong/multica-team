...
// codexModelProvider returns the provider name that must be supplied at the
// app-server thread boundary for a wrapped ducX runtime. The ducX CLI uses
// OneAPI, but its process-level `-c model_provider=oneapi` default is not
// carried into a thread/start request when the app-server is driven by an
// external client. Keep this targeted to the ducX executable so ordinary
// Codex runtimes retain their native provider selection.
func codexModelProvider(cfg Config) string {
	executable := strings.ToLower(filepath.Base(strings.TrimSpace(cfg.ExecutablePath)))
...
func (c *codexClient) startOrResumeThread(ctx context.Context, opts ExecOptions, logger *slog.Logger) (string, bool, error) {
	modelProvider := nilIfEmpty(codexModelProvider(c.cfg))
