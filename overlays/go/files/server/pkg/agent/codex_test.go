	...
	wait := drainRPCScript(t, c, fs, []rpcResponse{
		{
			...
			assertFn: func(t *testing.T, params map[string]any) {
				...
				if params["modelProvider"] != nil {
					t.Errorf("modelProvider = %v, want nil for the ordinary Codex executable", params["modelProvider"])
				}
...
func TestCodexStartOrResumeThreadUsesOneAPIForDucx(t *testing.T) {
	t.Parallel()

	c, fs, _ := newTestCodexClient(t)
	c.cfg.ExecutablePath = "/root/.multica/shims/ducx"

	wait := drainRPCScript(t, c, fs, []rpcResponse{
		{
			method: "thread/start",
			result: json.RawMessage(`{"thread":{"id":"thr_ducx"}}`),
			assertFn: func(t *testing.T, params map[string]any) {
				if params["model"] != "grok-4.6" {
					t.Errorf("model = %v, want grok-4.6", params["model"])
				}
				if params["modelProvider"] != "oneapi" {
					t.Errorf("modelProvider = %v, want oneapi", params["modelProvider"])
				}
			},
		},
	})
	...
	threadID, resumed, err := c.startOrResumeThread(
		context.Background(),
		ExecOptions{Cwd: "/work", Model: "grok-4.6"},
		slog.Default(),
	)
	if err != nil {
		t.Fatalf("startOrResumeThread: %v", err)
	}
	if threadID != "thr_ducx" {
		t.Errorf("threadID = %q, want thr_ducx", threadID)
	}
	if resumed {
		t.Error("resumed should be false when no prior session is provided")
	}
