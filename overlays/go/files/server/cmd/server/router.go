...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	// Local overlay: a successful web login is what provisions the daemon
	// profile for this machine (per-user PAT + ~/.multica profile). The Bridge
	// reads those profiles, so users never have to install the CLI or repeat a
	// binding step for the shared bridge to see them.
	r.With(authVerifyRL).Post("/auth/verify-code", h.ProvisionDaemonAccessAfterLogin(h.VerifyCode))
	...
	r.Group(func(r chi.Router) {
		...
		r.With(handler.RequireHumanActor).Post("/api/daemon-bootstrap-token", h.GetOrCreateDaemonBootstrapToken)
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/api/skills", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					r.Get("/", h.GetSkill)
					r.Put("/", h.UpdateSkill)
					r.Put("/scope", h.ChangeSkillScope)
