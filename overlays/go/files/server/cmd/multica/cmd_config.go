...
func runConfigShow(cmd *cobra.Command, _ []string) error {
	...
	fmt.Fprintf(os.Stdout, "%-34s %s\n", "daemon_runtimes:", strings.Join(cfg.DaemonRuntimes, ","))
	fmt.Fprintf(os.Stdout, "%-34s %s\n", "runtime_shims:", strings.Join(cfg.RuntimeShims, ","))
...
func applyConfigSet(cfg *cli.CLIConfig, key, value string) error {
	...
	case "daemon_runtimes":
		// Comma or space separated provider ids; empty clears the restriction.
		value = strings.TrimSpace(value)
		if value == "" {
			cfg.DaemonRuntimes = nil
			return nil
		}
		...
		if len(ids) == 0 {
			return fmt.Errorf("daemon_runtimes needs at least one provider id")
		}
		cfg.DaemonRuntimes = ids
	case "runtime_shims":
		// Comma or space separated runtime command names; empty clears the list.
		// Each name gets a generated wrapper under ~/.multica/shims/ that the
		// daemon launches instead of the bare command.
		value = strings.TrimSpace(value)
		if value == "" {
			cfg.RuntimeShims = nil
			return nil
		}
		parts := strings.FieldsFunc(value, func(r rune) bool { return r == ',' || r == ' ' })
		...
		for _, part := range parts {
			...
			// A name becomes a file name under ~/.multica/shims/, so keep it to
			// the shape a PATH lookup would accept.
			if strings.ContainsAny(name, "/\\") || name == "." || name == ".." {
				return fmt.Errorf("runtime_shims takes command names, not paths: %q", name)
			}
		...
		if len(names) == 0 {
			return fmt.Errorf("runtime_shims needs at least one command name")
		}
		cfg.RuntimeShims = names
