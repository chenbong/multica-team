...
type AgentResponse struct {
	...
	RuntimeSkillsDefaultEnabled      bool                   `json:"runtime_skills_default_enabled"`
...
func (h *Handler) agentToResponse(a db.Agent) AgentResponse {
	...
	return AgentResponse{
		...
		RuntimeSkillsDefaultEnabled: a.RuntimeSkillsDefaultEnabled,
...
type TaskAgentData struct {
	...
	RuntimeSkillsDefaultEnabled *bool                       `json:"runtime_skills_default_enabled,omitempty"`
...
func (h *Handler) ListAgents(w http.ResponseWriter, r *http.Request) {
	...
	skillRows, err := h.Queries.ListAgentSkillsByWorkspace(r.Context(), db.ListAgentSkillsByWorkspaceParams{WorkspaceID: parseUUID(workspaceID), OwnerUserID: parseUUID(userID)})
	if err != nil {
...
func (h *Handler) CreateAgent(w http.ResponseWriter, r *http.Request) {
	...
	// New user-authored agents start with the sandbox marker enabled. This is
	// persisted in the agent's own environment map, so the owner can change or
	// delete it later from the agent Environment Variables settings.
	if req.CustomEnv == nil {
		req.CustomEnv = make(map[string]string)
	}
	if _, exists := req.CustomEnv["IS_SANDBOX"]; !exists {
		req.CustomEnv["IS_SANDBOX"] = "1"
	}
	...
	for _, skillID := range skillUUIDs {
		skill, err := h.Queries.GetSkillForAgentBinding(r.Context(), db.GetSkillForAgentBindingParams{ID: skillID, WorkspaceID: wsUUID, OwnerUserID: parseUUID(requestUserID(r))})
		if err != nil || (skill.Scope == "personal" && uuidToString(parseUUID(ownerID)) != requestUserID(r)) {
			writeError(w, http.StatusBadRequest, "skill does not belong to this workspace or is not owned by this user")
			return
		}
	}
