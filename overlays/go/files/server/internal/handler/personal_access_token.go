...
func (h *Handler) GetOrCreateDaemonBootstrapToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to begin token transaction")
		return
	}
	defer tx.Rollback(r.Context())
	var encrypted []byte
	err = tx.QueryRow(r.Context(), `SELECT daemon_bootstrap_token_encrypted FROM "user" WHERE id=$1 FOR UPDATE`, owner).Scan(&encrypted)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to load daemon token")
		return
	}
	...
	if len(encrypted) > 0 {
		...
		if err != nil {
			writeError(w, http.StatusInternalServerError, "failed to decrypt daemon token")
			return
		}
		raw = string(plain)
		// Validate both the encrypted payload and its PAT owner before returning it.
		pat, err = q.GetPersonalAccessTokenByHash(r.Context(), auth.HashToken(raw))
		if err != nil && !errors.Is(err, pgx.ErrNoRows) {
			writeError(w, http.StatusInternalServerError, "failed to validate daemon token")
			return
		}
		if err == nil && (pat.UserID != owner || !strings.HasPrefix(raw, "mul_")) {
			writeError(w, http.StatusInternalServerError, "daemon token owner mismatch")
			return
		}
	...
	if raw == "" {
		...
		if err != nil {
			writeError(w, http.StatusInternalServerError, "failed to encrypt daemon token")
			return
		}
		pat, err = q.CreatePersonalAccessToken(r.Context(), db.CreatePersonalAccessTokenParams{
		...
		if _, err = tx.Exec(r.Context(), `UPDATE "user" SET daemon_bootstrap_token_encrypted=$1 WHERE id=$2`, sealed, owner); err != nil {
			writeError(w, http.StatusInternalServerError, "failed to store daemon token")
			return
		}
	}
	if err = tx.Commit(r.Context()); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to commit daemon token")
		return
	}
