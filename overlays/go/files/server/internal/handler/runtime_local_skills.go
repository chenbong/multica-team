...
func (s *InMemoryLocalSkillImportStore) Create(_ context.Context, input LocalSkillImportRequestInput) (*RuntimeLocalSkillImportRequest, error) {
	...
	scope := input.Scope
	// Team scope has to survive here: the request is created by the skill
	// dialog, and a silent downgrade to "workspace" is exactly what made an
	// import started as a team skill land in the current project.
	if scope != "personal" && scope != "workspace" && scope != "team" {
		scope = "workspace"
	}
...
func (h *Handler) ReportLocalSkillImportResult(w http.ResponseWriter, r *http.Request) {
	...
	if existing, found, lerr := h.lookupSkillByName(r.Context(), rt.WorkspaceID, parseUUID(req.CreatorID), scope, sanitizeNullBytes(name)); lerr != nil {
	...
	if err != nil {
		...
		if isUniqueViolation(err) {
			if existing, found, lerr := h.lookupSkillByName(r.Context(), rt.WorkspaceID, parseUUID(req.CreatorID), scope, sanitizeNullBytes(name)); lerr == nil && found {
				h.resolveLocalSkillConflict(w, r, req, existing)
				return
			}
	...
	if err := h.LocalSkillImportStore.Complete(r.Context(), requestID, resp); err != nil {
		...
		if delErr := h.Queries.DeleteSkill(r.Context(), db.DeleteSkillParams{
			...
			WorkspaceID: rt.WorkspaceID,
			OwnerUserID: parseUUID(req.CreatorID),
		}); delErr != nil {
...
// lookupSkillByName resolves a skill by its level identity. found=false with
// a nil error means there is no such skill — i.e. no conflict.
func (h *Handler) lookupSkillByName(ctx context.Context, workspaceID, ownerUserID pgtype.UUID, scope, name string) (db.Skill, bool, error) {
