...
func (h *Handler) importSkillFromArchive(w http.ResponseWriter, r *http.Request, workspaceID string, workspaceUUID, creatorUUID pgtype.UUID, creatorID string) {
	...
	// The URL-import branch of this same route already accepts "team"; the
	// archive branch used to reject it, so importing a .skill/.zip after
	// choosing "team" in the create dialog failed instead of creating a team
	// skill.
	if scope != "workspace" && scope != "personal" && scope != "team" {
	...
	h.finishSkillImport(w, r, workspaceID, workspaceUUID, creatorUUID, creatorID, strategy, true, imported, scope)
}
