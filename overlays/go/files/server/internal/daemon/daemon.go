...
func (d *Daemon) appendProfileRuntimes(ctx context.Context, workspaceID string, runtimes *[]map[string]string, failedProfiles *[]map[string]string) string {
	...
	for _, profile := range resp.RuntimeProfiles {
		...
		// Local overlay: a listed command gets a generated wrapper (see
		// ensureRuntimeShim) so a vendor CLI that drops its credentials on the
		// exact flag set we pass still works. Unlisted commands, and hosts
		// without a shim list, keep the resolved path untouched.
		if shim := ensureRuntimeShim(profile.CommandName, resolved, d.logger); shim != "" {
			resolved = shim
		}
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	disabledRuntimeSkills, err := convertDisabledRuntimeSkillsForEnv(task.Agent, task.RuntimeID, provider)
	if err != nil {
		return TaskResult{}, fmt.Errorf("discover default-off runtime skills: %w", err)
	}
	taskCtx := execenv.TaskContextForEnv{
		...
		DisabledRuntimeSkills:            disabledRuntimeSkills,
...
func convertDisabledRuntimeSkillsForEnv(agentData *AgentData, runtimeID, provider string) ([]execenv.RuntimeSkillRefForEnv, error) {
	if agentData == nil {
		return nil, nil
	}

	defaultEnabled := true
	if agentData.RuntimeSkillsDefaultEnabled != nil {
		defaultEnabled = *agentData.RuntimeSkillsDefaultEnabled
	}

	// Existing agents and older servers use the original deny-list semantics.
	if defaultEnabled {
	...
	// A default-off agent stores only the skills the user explicitly enabled.
	// Discover the current runtime inventory locally and deny the rest. This
	// also makes skills installed after agent creation default to off.
	if provider != "codex" && provider != "claude" {
		return nil, nil
	}
	discovered, supported, err := listRuntimeLocalSkills(provider)
	if err != nil {
		return nil, err
	}
	if !supported {
	...
	enabled := make(map[string]struct{}, len(agentData.DisabledRuntimeSkills))
	for _, skill := range agentData.DisabledRuntimeSkills {
		if skill.RuntimeID != runtimeID || skill.Provider != provider || skill.Enabled == nil || !*skill.Enabled {
			continue
		}
		enabled[runtimeSkillOverrideKey(skill.Root, skill.Key, skill.Plugin)] = struct{}{}
	}
	result := make([]execenv.RuntimeSkillRefForEnv, 0, len(discovered))
	for _, skill := range discovered {
		if !skill.CanDisable {
			continue
		}
		if _, ok := enabled[runtimeSkillOverrideKey(skill.Root, skill.Key, skill.Plugin)]; ok {
			continue
		}
		result = append(result, execenv.RuntimeSkillRefForEnv{
			Root:   skill.Root,
...
func runtimeSkillOverrideKey(root, key, plugin string) string {
	return root + "\x00" + key + "\x00" + plugin
}
