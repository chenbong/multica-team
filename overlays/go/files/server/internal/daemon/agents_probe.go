...
// shim:
//
//	multica config set runtime_shims ducc      # or MULTICA_DAEMON_RUNTIME_SHIMS
//
// The daemon then writes ~/.multica/shims/<command>, which copies the settings
// file into the project settings the CLI reads on its own and launches the real
// command without that flag. Nothing else about the launch changes, and a
// command that is not listed is launched exactly as before — so the wrapper
// never touches a CLI that works.
...
// runtimeShimScript renders the wrapper. It only rewrites the one flag pair the
// CLI is known to choke on and is a pure pass-through for every other launch,
// including the version probe (`--version`).
