...
var probeAgentCLIs = func() map[string]AgentEntry {
	...
	// Local overlay: MULTICA_DAEMON_RUNTIMES stops the built-in probe from
	// advertising CLIs this deployment does not want as runtimes — a host may
	// have claude/codex installed for other reasons without wanting a runtime
	// row for each. Custom runtime profiles (e.g. ducc/ducx) are registered
	// through their own path and are not affected, so a deployment can expose
	// nothing but its own profiles. An unset or empty value keeps upstream's
	// behaviour of registering everything the machine has.
	return filterProbedAgents(agents)
}

// filterProbedAgents drops every probed provider that the deployment did not
// list in MULTICA_DAEMON_RUNTIMES. With no restriction configured it returns the
// map untouched, which is upstream's behaviour.
func filterProbedAgents(agents map[string]AgentEntry) map[string]AgentEntry {
	allow := daemonRuntimeAllowlist()
	if allow == nil {
		return agents
	}
	for id := range agents {
		if _, keep := allow[id]; !keep {
			delete(agents, id)
		}
	}
	return agents
...
// daemonRuntimeAllowlist parses MULTICA_DAEMON_RUNTIMES (comma separated
// provider ids such as "ducc,ducx"). A missing or empty value returns nil, which
// callers read as "no restriction".
func daemonRuntimeAllowlist() map[string]struct{} {
	raw := strings.TrimSpace(os.Getenv("MULTICA_DAEMON_RUNTIMES"))
	if raw == "" {
		return nil
	}
...
// --- runtime command shims -------------------------------------------------
//
// Some vendor CLIs misbehave with the exact argument set the agent backends
// hand them. The reference case is an internal Claude Code build (baidu-cc
// 2.1.258.3): passing --verbose together with --settings makes it drop its
// stored credentials and answer "Not logged in · Please run /login", while
// either flag alone works. --verbose cannot simply be dropped (the CLI rejects
// `-p --output-format stream-json` without it) and --settings carries the
// runtime's skill policy, so a deployment declares the offending command as a
// shim.
//
// The ducX case is different: its interactive `--yolo` flag does not become
// the app-server session policy that Multica starts. The shim therefore adds
// explicit Codex config overrides only when the command is launching
// `app-server`, making the full-access/no-approval behavior transparent to
// the agent and preserving ordinary probes such as `--version`.
//
//	multica config set runtime_shims ducc,ducx  # or MULTICA_DAEMON_RUNTIME_SHIMS
//
// The daemon then writes ~/.multica/shims/<command>, which applies the
// command-specific compatibility behavior and launches the real command.
// Nothing else about the launch changes, and a command that is not listed is
// launched exactly as before — so the wrapper never touches a CLI that works.

// daemonRuntimeShims parses MULTICA_DAEMON_RUNTIME_SHIMS (comma separated
// command names such as "ducc"). A missing or empty value returns nil, which
// callers read as "no shims".
func daemonRuntimeShims() map[string]struct{} {
	raw := strings.TrimSpace(os.Getenv("MULTICA_DAEMON_RUNTIME_SHIMS"))
	if raw == "" {
		return nil
	}
	shims := map[string]struct{}{}
	for _, part := range strings.Split(raw, ",") {
...
// runtimeShimDir keeps the generated wrappers next to the CLI profiles, out of
// the task workdir, so the agent cannot edit its own wrapper.
func runtimeShimDir() string {
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return ""
	}
	return filepath.Join(home, ".multica", "shims")
}

// ensureRuntimeShim writes (or refreshes) the wrapper for `command`, which
// resolved to the executable `real`, and returns the wrapper's path. It returns
// "" when the command is not listed in MULTICA_DAEMON_RUNTIME_SHIMS or when the
// wrapper cannot be written: callers then keep the real path, so the only
// consequence is the CLI behaving as it does without this overlay.
func ensureRuntimeShim(command, real string, logger *slog.Logger) string {
	shims := daemonRuntimeShims()
	if len(shims) == 0 || real == "" {
		return ""
	}
	if _, listed := shims[command]; !listed {
		return ""
	}
	dir := runtimeShimDir()
	if dir == "" {
	...
	if err := os.MkdirAll(dir, 0o700); err != nil {
		logger.Warn("runtime shim: cannot create directory", "dir", dir, "command", command, "error", err)
		return ""
	}
	path := filepath.Join(dir, command)
	script := runtimeShimScript(command, real)
	if current, err := os.ReadFile(path); err == nil && string(current) == script {
		return path
	}
	tmp := path + ".tmp"
	if err := os.WriteFile(tmp, []byte(script), 0o700); err != nil {
		logger.Warn("runtime shim: cannot write wrapper", "path", tmp, "error", err)
		return ""
	}
	if err := os.Rename(tmp, path); err != nil {
		logger.Warn("runtime shim: cannot install wrapper", "path", path, "error", err)
		_ = os.Remove(tmp)
		return ""
	}
	logger.Info("runtime shim installed", "command", command, "path", path, "real", real)
	return path
}

// runtimeShimScript renders the wrapper for one known runtime command. Each
// command gets the smallest compatibility change it needs and remains a pure
// pass-through for unrelated invocations, including the version probe.
func runtimeShimScript(command, real string) string {
	if command == "ducx" {
		return runtimeDucxShimScript(command, real)
	}
	return runtimeDuccShimScript(command, real)
}

// runtimeDuccShimScript removes the settings flag that breaks the internal
// Claude Code build when Multica also supplies --verbose, while preserving the
// settings through the project-local file that Claude reads automatically.
func runtimeDuccShimScript(command, real string) string {
	return `#!/usr/bin/env bash
# Generated by Multica for the "` + command + `" runtime — do not edit by hand;
# the daemon rewrites this file on every start and whenever the real command
# moves. ` + command + ` stops authenticating when --verbose and --settings are
# passed together (stream-json output requires --verbose, and --settings carries
# the runtime's skill policy), so the settings file is copied to the project
# settings the CLI reads on its own, and the flag is dropped from the launch.
set -eo pipefail
real=` + shellSingleQuote(real) + `
args=()
settings=""
while [ "$#" -gt 0 ]; do
  case "$1" in
...
if [ -n "$settings" ] && [ -f "$settings" ]; then
  ...
  if cp -f "$settings" .claude/settings.local.json 2>/dev/null; then
    chmod 600 .claude/settings.local.json 2>/dev/null || true
  fi
fi
exec "$real" ${args[@]+"${args[@]}"}
`
...
// runtimeDucxShimScript turns the app-server invocation into the same
// full-access/no-approval execution mode verified for the direct ducX CLI.
// ducX's --yolo flag is not reflected in the app-server thread/start defaults,
// so the config overrides are appended after the caller's arguments. Codex's
// last-wins config semantics make the shim authoritative without changing the
// runtime profile or the agent visible command.
func runtimeDucxShimScript(command, real string) string {
	return `#!/usr/bin/env bash
# Generated by Multica for the "` + command + `" runtime — do not edit by hand;
# the daemon rewrites this file on every start and whenever the real command
# moves. The app-server path needs explicit session policy overrides: ducX's
# interactive --yolo flag is not carried into thread/start.
set -eo pipefail
real=` + shellSingleQuote(real) + `
args=("$@")
app_server=false
...
// shellSingleQuote renders value as a single-quoted shell word.
