...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if cliCfg, err := cli.LoadCLIConfigForProfile(overrides.Profile); err != nil {
		...
		// Local overlay: a profile can restrict which built-in CLIs this daemon
		// advertises as runtimes. The value is published as the env var the
		// probe reads, so the startup probe and the periodic re-probe agree.
		applyDaemonRuntimesOverride(cliCfg.DaemonRuntimes)
		// Local overlay: same idea for the commands that need a generated
		// wrapper before launch (see ensureRuntimeShim in agents_probe.go).
		applyRuntimeShimsOverride(cliCfg.RuntimeShims)
	...
	// Local overlay: an explicit MULTICA_DAEMON_RUNTIMES allowlist is a
	// deliberate statement about which built-in runtimes this host advertises,
	// so an empty result is a valid configuration — it is exactly what a
	// deployment that only wants its own custom runtime profiles (ducc/ducx)
	// looks like, because those names are not part of the built-in probe at
	// all. Without an allowlist the upstream check is untouched: nobody should
	// silently end up with a daemon that has no runtime available.
	if len(agents) == 0 && !overrides.AllowNoAgents && daemonRuntimeAllowlist() == nil {
...
// applyDaemonRuntimesOverride publishes the profile's daemon_runtimes list as
// MULTICA_DAEMON_RUNTIMES, unless the operator already exported it (env wins
// over config, the same precedence the OpenClaw overrides use). An empty list
// keeps upstream's behaviour: probe and advertise every CLI found.
func applyDaemonRuntimesOverride(runtimes []string) {
	if len(runtimes) == 0 {
		return
	}
	if _, set := os.LookupEnv("MULTICA_DAEMON_RUNTIMES"); set {
		return
	}
	...
	if len(ids) > 0 {
		_ = os.Setenv("MULTICA_DAEMON_RUNTIMES", strings.Join(ids, ","))
	}
}

// applyRuntimeShimsOverride publishes the profile's runtime_shims list as
// MULTICA_DAEMON_RUNTIME_SHIMS, with the same precedence as the runtime
// allowlist above: an explicitly exported variable wins, and an empty list
// means "no wrappers", which launches every runtime command unmodified.
func applyRuntimeShimsOverride(shims []string) {
	if len(shims) == 0 {
		return
	}
	if _, set := os.LookupEnv("MULTICA_DAEMON_RUNTIME_SHIMS"); set {
		return
	}
	...
	if len(names) > 0 {
		_ = os.Setenv("MULTICA_DAEMON_RUNTIME_SHIMS", strings.Join(names, ","))
	}
}
