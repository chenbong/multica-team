...
type CLIConfig struct {
	...
	// DaemonRuntimes, when non-empty, restricts which built-in provider CLIs
	// this daemon advertises as runtimes. It exists so a deployment can keep a
	// host's claude/codex installs out of its runtime list while still running
	// its own custom runtime profiles. Set it with
	// `multica config set daemon_runtimes ducc,ducx`; empty (the default) keeps
	// upstream's behaviour of registering every CLI found on the machine.
	DaemonRuntimes []string `json:"daemon_runtimes,omitempty"`

	// RuntimeShims, when non-empty, names custom runtime commands that need a
	// generated wrapper before the daemon launches them. The wrapper exists
	// because some vendor CLIs misbehave with the exact argument set Multica
	// hands them — the reference case is a CLI that drops its credentials when
	// --verbose and --settings are passed together. Multica writes one small
	// script per name under ~/.multica/shims/ and launches that instead of the
	// bare command, so the workspace's runtime profile does not have to change.
	// Set it with `multica config set runtime_shims ducc`; empty (the default)
	// launches every runtime command exactly as it is.
	RuntimeShims []string `json:"runtime_shims,omitempty"`
