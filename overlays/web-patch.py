...
function daemonCommands(
  ...
  if (normalizedServerUrl && normalizedAppUrl) {
    return {
      setupCmd: `multica${p} setup self-host --server-url ${normalizedServerUrl} --app-url ${normalizedAppUrl}__RUNTIME_LINE__`,
      tokenCmd: `multica${p} config set server_url ${normalizedServerUrl}
multica${p} config set app_url ${normalizedAppUrl}
multica${p} login --token ${cliToken || "<YOUR_TOKEN>"}__RUNTIME_LINE__
multica${p} daemon start`,
    };
  }

  return {
    setupCmd: `multica${p} setup__RUNTIME_LINE__`,
